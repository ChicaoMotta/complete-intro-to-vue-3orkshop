...
<template>
  <h2>Statistics</h2>
  <ul>
    <li v-for="(stat, type) in benderStatistics" :key="`${stat}-${type}`">{{ type }}: {{ stat }}</li>
  </ul>
  <h2>Characters</h2>
  <p v-if="characterList.length === 0">There are no characters</p>
  <ul v-else-if="characterList.length % 2 === 0">
    <li v-for="character in characterList" :key="`${character}`">
      <p>{{ character.name }}</p>
  ...
  <ul v-if="favoriteList.length > 0">
    <li v-for="character in favoriteList" :key="`${character}`">{{ character }}</li>
  </ul>
  ...
  <p>
    <span v-for="(character, index) in characterList" :key="`${index}`">{{ character.name }}{{ index ===
